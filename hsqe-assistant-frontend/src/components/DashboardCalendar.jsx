...
// RBC all-day: end should be next day 00:00 (exclusive)
...
function Toolbar({ label, onNavigate, onView, view, date }) {
  ...
  const prettyLabel = formatRangeLabel(label);
    ...
    >
      ...
      <div style={{ display: "inline-flex", gap: 10, alignItems: "center", minWidth: 0 }}>
        ...
        >
          {(() => {
            void tick;
            return formatHMSS(new Date());
          })()}
        </div>
      </div>

      <div style={{ display: "inline-flex", gap: 8, alignItems: "center", flex: "0 0 auto" }}>
        <button type="button" style={btn(view === Views.DAY)} onClick={() => onView(Views.DAY)}>
          Day
        </button>
        <button type="button" style={btn(view === Views.WEEK)} onClick={() => onView(Views.WEEK)}>
          Week
        </button>
        <button type="button" style={btn(view === Views.WORK_WEEK)} onClick={() => onView(Views.WORK_WEEK)}>
          Work week
        </button>
        <button type="button" style={btn(view === Views.MONTH)} onClick={() => onView(Views.MONTH)}>
          Month
        </button>
      </div>
...
export default function DashboardCalendar({ certificates = [], tasks = [], user }) {
  ...
  React.useEffect(() => {
    ...
    function onMove(e) {
      const el = e.target;
      if (!el?.closest?.(".rbc-event")) hideTooltip();
    }
    function onScroll() {
      hideTooltip();
    }
  ...
  function slotPropGetter(slotDate) {
    ...
    return {
      style: {
        background: isWorkHour ? "rgba(15, 23, 42, 0.08)" : "rgba(15, 23, 42, 0.02)",
      },
    };
  }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <div style={chkWrap}>
          <label style={filterItem(filters.certsDue, COLOR_CERT)}>
            <input
              type="checkbox"
              checked={filters.certsDue}
              onChange={(e) => setFilters((p) => ({ ...p, certsDue: e.target.checked }))}
              style={{ margin: 0 }}
            />
            <ScrollText size={16} />
            Certificates Due
          ...
          <label style={filterItem(filters.tasksReminder, COLOR_TASK)}>
            <input
              type="checkbox"
              checked={filters.tasksReminder}
              onChange={(e) => setFilters((p) => ({ ...p, tasksReminder: e.target.checked }))}
              style={{ margin: 0 }}
            />
            <CheckCheck size={16} />
            Task Reminders
          ...
          <label style={filterItem(filters.tasksDue, COLOR_TASK)}>
            <input
              type="checkbox"
              checked={filters.tasksDue}
              onChange={(e) => setFilters((p) => ({ ...p, tasksDue: e.target.checked }))}
              style={{ margin: 0 }}
            />
            <BookX size={16} />
            Tasks Due
          ...
          <label style={filterItem(filters.notesReminder, COLOR_NOTE)}>
            <input
              type="checkbox"
              checked={filters.notesReminder}
              onChange={(e) => setFilters((p) => ({ ...p, notesReminder: e.target.checked }))}
              style={{ margin: 0 }}
            />
            <StickyNote size={16} />
            Note Reminders
        ...
        >
          ...
          <div style={{ display: "grid", gap: 4 }}>
            {hover.lines.map((l, idx) => (
              <div key={idx} style={{ fontSize: 12, color: "#475569", fontWeight: 800, overflowWrap: "anywhere" }}>
                {l}
              </div>
            ))}
          </div>
